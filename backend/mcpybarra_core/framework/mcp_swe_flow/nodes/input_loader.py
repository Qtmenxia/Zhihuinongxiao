...
from mcp_swe_flow.utils import find_api_file, load_api_spec, load_mcp_doc
from mcp_swe_flow.config import (
...
"""
    ...
    if mode == "api":
        ...
        # 用户输入模式：确保用户输入有效
        if not user_input or len(user_input.strip()) < 10:  # 简单的有效性检查
