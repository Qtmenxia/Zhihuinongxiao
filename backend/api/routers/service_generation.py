...
async def get_service_status(
    ...
    if service.status == ServiceStatus.GENERATING:
        try:
            ...
            logger.error(f"Failed to get progress for service {service_id}: {e}", exc_info=True)
            pass
...
async def call_service_tool(
    ...
    try:
        ...
        log_entry = ServiceLog(
            id=f"log_{service_id}_{datetime.now(timezone.utc).replace(tzinfo=None).timestamp()}",
            service_id=service_id,
            tool_name=tool_name,
            input_params=params,
